...
- Expanded TUI startup, reconnect, resize/reflow, terminal palette/focus, Vim editing, and session recovery paths with real PTY/VT100 Gate B scenarios and stable integration suites.
- Aligned the CLI working-directory flags with Codex using `--cd` and `-C`, and extended CLI/TUI structure inventory, current-fixture guards, and npm launcher tests.
...
- Fixed TUI history hydration, disconnected editing, terminal restoration, Unicode cursor handling, narrow-terminal wrapping, queue recovery, multi-agent projection, and Vim search boundaries.
...
- Version consistency, TypeScript typecheck, protocol contracts, script governance, targeted CLI/TUI Rust tests, CLI Gate B, TUI Gate B, and the macOS arm64 npm packaged Gate B passed.
...
- Bumped the root app, CLI npm package, Rust workspace, and Cargo.lock versions to `1.144.0`.
...
**Full changes**: `v1.143.0` -> `v1.144.0`
