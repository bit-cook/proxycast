    ...
    while cursor < input.len() {
        if inside_paste {
            if input[cursor..].starts_with(PASTE_END) {
                ...
                let color_slot = [(10, b"\x1b]10;"), (11, b"\x1b]11;")]
                    .into_iter()
                    .find(|(_, prefix)| input[cursor..].starts_with(*prefix));
                let color_response = color_slot.and_then(|(slot, _)| {
                    super::osc_payload_end(
                        &input[cursor + 2..input.len().min(cursor + MAX_REPLAYED_OSC_BYTES)],
                    )
                    .and_then(|(end, terminator_len)| {
                        ...
                        .then_some(2 + end + terminator_len)
                    })
                });
                if let Some(response_len) = color_response {
                    cursor += response_len;
                } else {
