...
impl HyperlinkCache {
    ...
    pub(super) fn mark(
        ...
        for (row, index) in visible_rows.enumerate() {
            let Some(line_range) = lines.get(index) else {
                break;
            };
            let source_column = self.source_columns.get(index).copied().unwrap_or_default();
            ...
            if !annotated.hyperlinks.is_empty() {
                ...
                mark_buffer_hyperlinks(buf, row_area, &[annotated], 0);
            }
        }
    }
}
