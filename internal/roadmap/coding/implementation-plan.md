...
- `cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server runtime::tests::coding_events --no-run`
...
当前状态：`completed for P2 current owner / P2-A bounded output boundary / P2-B sandbox-aware canonical command-test owner / P2-C Windows runner owner`。P2 主干已接入 runtime tool event mirror、真实 `patch-apply` apply service、per-file checkpoint/content/diff refs、policy/sandbox/action facts、output snapshot、history hydrate 与 active command/test/action read model；artifact/checkpoint refs 继续由 RuntimeCore read-model 边界承接。Agent 前台 `exec_command` 已通过 `RuntimeToolExecutionAttempt` 进入 unified exec 与 App Server shared process owner，macOS seatbelt 与 Linux bubblewrap 在可用时进入 current runner；Windows 已接入双 setup mode、restricted-token runner、ACL/Job/ConPTY、Firewall/WFP、readiness 和 warning lifecycle。P2-A 已让 pipe、PTY、Seatbelt、Bubblewrap、Windows restricted runner 与远端 Environment 共享 1 MiB head/tail、精确 omission、64-slot broadcast、UTF-8 framing、unified exec bounded observation 与逐进程 replay。下一步直接进入当前工作树的 verifier/distribution evidence；Windows schema v3 `7/7` 只作历史平台基线，当前 SHA 的大输出仍需 Windows 真机复验。
...
- `tool-runtime::sandbox` 已补分平台 sandbox backend plan owner：按 macOS / Linux / Windows / unsupported 映射 `seatbelt / linux_sandbox / restricted_token / none`，输出 `sandboxBackend/sandboxBackendStatus/sandboxBackendEnforced/sandboxBackendReasonCode/sandboxBackendPlatform/workspaceSandbox*` diagnostics；macOS `sandbox-exec` 与 Linux `bwrap` 可用时标记 `ready/enforced=true`。Windows restricted-token runner 已通过 typed `LocalExecutionSandbox` 接入 current execution process owner；setup mode 明确区分 elevated sandbox-account 与 unelevated current-user runner，ACL/Job/ConPTY、Firewall/WFP、readiness read-back 与 warning lifecycle 由同一 owner 承接，Windows schema v3 `7/7` evidence 支撑 `Ready/enforced=true`。生产决策仍保持 fail closed，不静默回退无沙箱。
...
- 2026-09-06 P2-B command/exec owner 收口增量：App Server `command/exec` 现在启动后立即注册到共享 `ExecutionProcessServer`，stdout/stderr delta、终态 snapshot、超时终止、stdin 写入和 connection close 都复用同一 process owner；公开 `processId` 仍按 connection scope 复用，内部 owner ID 以 `command-exec-{connectionId}-{processId}` 为 base，终态 owner 已保留时追加 instance UUID，避免跨连接冲突和终态复用冲突。重复 session 在启动前拒绝，resize 或 owner 注册失败会终止已启动进程；新增测试验证 shared status/output replay、跨连接同名 process ID 和终态后复用公开 process ID。
- 2026-09-06 P2-B command/exec owner 验证证据：`rustfmt --edition 2021 "lime-rs/crates/app-server/src/command_exec.rs" "lime-rs/crates/app-server/src/command_exec/tests.rs"` 与 `git diff --check -- "lime-rs/crates/app-server/src/command_exec.rs" "lime-rs/crates/app-server/src/command_exec/tests.rs"` 通过；`cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server command_exec --lib -- --nocapture` 与 `cargo test --manifest-path "lime-rs/Cargo.toml" -p tool-runtime execution_process -- --nocapture` 均被本机 `rusty_v8 v150.4.0` Darwin arm64 archive HTTP 404 阻断，未计为通过。
- 2026-09-06 P2-B unified exec command/test lifecycle 增量：`tool-runtime::unified_exec` 现在把同一 `ExecutionProcessSnapshot` 的 `processId / executionProcessStatus / stdinWritable / outputBytes / outputOmittedBytes / outputTruncated / exit_code` 与 `executionSurface=unified_exec` 写入每次 canonical result；短命令、yield 后 running 命令和 `write_stdin` 续接保持同一 process identity。App Server `runtime_backend::coding_events::CodingEventMirror` 已消费 `Command` Item 的 `ItemStarted -> ItemUpdated -> ItemCompleted`，将统一 exec 产出为 `command.started/output/exited` 与 `test.started/completed`，并保留 bounded output/process metadata；不新增 JSON-RPC method，不绕过 sandbox executor。
- 2026-09-06 P2-B unified exec command/test lifecycle 验证证据：`rustfmt --edition 2021 --check "lime-rs/crates/tool-runtime/src/unified_exec.rs" "lime-rs/crates/tool-runtime/src/unified_exec/tests.rs" "lime-rs/crates/app-server/src/runtime_backend/coding_events.rs" "lime-rs/crates/app-server/src/runtime_backend/coding_events/tests.rs"` 与对应 `git diff --check` 通过；新增 unified exec snapshot metadata 与 `Command` Item 生命周期回归。`cargo test --manifest-path "lime-rs/Cargo.toml" -p tool-runtime unified_exec --lib -- --nocapture`、`cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server runtime_backend::coding_events --lib -- --nocapture` 均被本机 `rusty_v8 v150.4.0` Darwin arm64 archive HTTP 404 阻断，未计为通过。
- 2026-09-06 P2-B command lifecycle projection correction：`CodingEventMirror` 统一 `Command` 的 `commandId`（包括 terminal interaction），对无 `ItemStarted` 的冷恢复终态补发 started/output/exited，并将 `Cancelled`/`Interrupted`/aborted/terminated 映射为 `canceled`（含 `test.completed`）。新增 call-id、一致性、冷恢复和取消回归；未新增协议或 owner。
- 2026-09-06 P2-B thread shell live output 收口：`thread/shellCommand` 继续复用共享 `ExecutionProcessServer`，轮询改为按 `processId + afterSequence` drain bounded output，并在同一 turn 内即时落 `command.output`；终态仍由同一 snapshot 生成 `command.exited`，启动/输出/终态统一携带 `executionProcessStatus / executionProcessControlStatus / executionSurface / stdinWritable / outputBytes` 等 owner facts。显式 `status=canceled` 优先于 exit code，避免 terminated shell 被 Thread Item 投影为 failed/completed。新增 session shell lifecycle、delta、process-control metadata 回归；未新增 JSON-RPC method、legacy owner 或裸进程路径。
- 2026-09-06 P2-B thread shell live output 验证证据：`rustfmt --edition 2021 --check "lime-rs/crates/app-server/src/runtime/session_shell.rs" "lime-rs/crates/app-server/src/runtime/thread_item_projection/coding_items.rs" "lime-rs/crates/app-server/src/runtime/tests/session_shell.rs"`、对应 `git diff --check`、`npm run test:contracts` 与 `npm run governance:legacy-report` 通过；`cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server runtime::tests::session_shell --lib -- --nocapture` 仍被本机 `rusty_v8 v150.4.0` Darwin arm64 archive HTTP 404 阻断，未计为通过。
- 2026-09-06 P2-B unified exec live output bridge 增量：`RuntimeToolExecutorHandle::execute_call` 将 canonical `ToolCall` 的生命周期 emitter 注入 `RuntimeToolExecutionContext`，`tool-runtime::unified_exec` 在每次 `processId + afterSequence` drain 后按原序发出 `ToolOutputDeltaEvent`，并把 `outputSequence / outputKind / outputBytes / outputOmittedBytes / outputTruncated / executionProcessStatus / stdinWritable / executionSurface` 与 terminal snapshot 对齐；`write_stdin` 续接继续使用原始 exec call/turn identity，`CodingEventMirror` 对已流式输出的终态抑制聚合 output 重复，不新增 Agent 依赖、JSON-RPC method 或第二套 output owner。
- 2026-09-06 P2-B unified exec live output bridge 验证证据：`rustfmt --edition 2021 --check` 覆盖 `tool_call.rs`、`tool_executor.rs`、`unified_exec.rs`、`unified_exec/tests.rs`、current orchestration 与 App Server unified exec fixtures，`git diff --check` 通过；unified exec fixture 新增短命令 `started -> output -> completed` metadata/call-id/sequence 回归。`cargo check/test` 仍被本机 `rusty_v8 v150.4.0` Darwin arm64 archive HTTP 404 阻断，未计为 Rust 通过。
- 2026-09-06 P2-B sandbox attempt control-owner 回归：`app-server/src/execution_process/tests.rs` 新增带 `RuntimeToolExecutionAttempt` 的 `workspace-write` 交互命令 fixture，证明 sandbox attempt 通过 `ExecutionProcessServer` 同一 current owner 完成启动 snapshot、按 process identity drain output、stdin 写入、终态 status，并核对 `tool_id` 与 attempt call identity；结合 unified exec/current orchestration wiring，P2-B canonical command/test owner 迁移证据闭环。`rustfmt --edition 2021 --check` 与 `git diff --check` 通过；`cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server execution_process --lib` 仍被本机 `rusty_v8 v150.4.0` Darwin arm64 archive HTTP 404 阻断，未计为 Rust 通过。
- 2026-09-06 P2-A bounded output boundary 收口：新增 `tool-runtime::execution_process::BoundedProcessOutput` 与 `ProcessOutputFramers` current owner。默认保留 1 MiB head/latest tail、精确统计 omitted bytes 并插入 omission marker；pipe、PTY、Seatbelt、Bubblewrap、Windows restricted runner 与远端 Environment 统一从 raw bytes 捕获，UTF-8 scalar 可跨 reader chunk，EOF flush 不完整后缀。reader -> handle 改为 64-slot bounded broadcast；unified exec observation 不再无界增长；App Server replay 下沉到逐进程 queue，远端输出删除手写 tail-only 截断。
- 2026-09-06 P2-A 验证证据：`npm run test:rust:unit -- -p tool-runtime execution_process` 28/28，`npm run test:rust:unit -- -p tool-runtime unified_exec` 11/11，`npm run test:rust:unit -- -p app-server execution_process` 20/20；覆盖 1 MiB head/tail、精确 omission marker、未消费 delta 的大输出 terminal completion、远端 snapshot 一致性、逐进程 replay 隔离与跨 chunk UTF-8。Windows 当前 SHA 的 restricted-token 真机大输出尚未复跑，不提升为平台 evidence。
- 未完成：当前工作树的 Pier/DeepSWE verifier 与正式 distribution evidence，以及 Windows 当前 SHA 的 restricted-token 大输出真机复验。外部 Agent host 消费 `process_control` 合同只在真实生产 adapter 出现时继续；network/policy 单条规则深链属于 P7/P8 增强。P2-A/P2-B/P2-C current owner、P3/P4 UI evidence、Provider slot diagnostics 与 GUI coding input baseline 均不再作为主线 blocker。
...
当前状态：`in_progress / change-output-log-action-recovery-session current projection connected / runtime hook split / reliability diagnostics policy-network evidence connected / execution policy recovery settings connected / recovery-gui-smoke-evidence connected / gui-coding-input-smoke-aggregated / read-model-artifact-refs connected / network-rule-recovery-draft connected / provider-focus-recovery connected / policy-source-layer-visibility connected / policy-field-source-reliability connected / sandbox-backend-diagnostics connected`。`CanvasWorkbenchLayout` 仍是承载壳，`WorkspaceConversationSceneRuntime` 已把 session、change、output、log views 从 `CodingWorkbenchView` 派生；Workbench、Reliability diagnostics 和继续修复动作均消费 current RuntimeEvent/thread_read projection，不直连 App Server、不新增命令、不依赖 mock。artifact/checkpoint refs、policy/sandbox/network diagnostics、provider focus 和 execution-policy recovery 均保留在 current read model / diagnostics 边界；旧 Evidence Pack provider/export summary 不再是 current owner。下一刀必须先在 `tool-runtime` 实现 Windows restricted-token runner，再在 Windows 真机补 enforce smoke；跨平台 GUI smoke 证据只能验证 current readiness 投影，不能替代 Windows 平台证据。
...
- 2026-06-15 Evidence Pack coding summary 记录已被 superseded：旧 provider/test surface 已删除；Coding Workbench 当前只以 RuntimeEvent、`thread_read`/read model 与 artifact refs 作为复盘事实，不得把旧 summary 或其测试命令视为 current 验收。
...
- 2026-08-10 current 代码复核更正：以下 2026-06-15 记录指向已删除的旧 owner、旧测试入口，并曾把 Windows plan 写成 `ready/enforced=true`，因此只保留为 superseded historical evidence，不得作为 current owner 或平台交付证据。当前 runner foundation 已在 `tool-runtime::execution_process::windows` 重建并由 typed `LocalExecutionSandbox` 接线；`prepare_sandbox_command` 拒绝 `RestrictedToken` 是因为该 backend 走专用进程 runner，不是 runner 缺失。该历史段落已被后续 Windows runner schema v3 `7/7`、GitHub quality run 与 readiness 复验 run supersede；当前 Windows plan 在真实 Windows runner 上返回 `Ready/enforced=true`，macOS 不能替代 Windows evidence。
...
- 2026-06-15 P2-B Workbench process control 增量：`src/lib/api/appServer.ts` 已补齐 App Server current `executionProcess/start|writeStdin|interrupt|terminate|status|drainOutput` client 方法，`src/lib/api/executionProcess.ts` 作为唯一前端领域网关返回 `response.result`，不走 `safeInvoke` 旧命令或 mock fallback；`CodingWorkbenchOutputPanel` 只对 `processId + running/starting` 的 `CodingWorkbenchView.commands` 渲染 icon controls，并由 `workspaceConversationCodingViews.tsx` 注入 interrupt / terminate / status / drain handlers。该增量与后续 unified exec/current orchestration 接线共同完成 command/test 默认 lifecycle、stdin 与 status hydrate 的 current owner 收敛。
...
- README 口径已同步：P1/P2/P3/P4/P5/P7/P8 骨架闭环具备 current facts、projection、GUI smoke、evidence export 与生产 mock / legacy command 防回流守卫；后续最高价值下一刀先在 `tool-runtime` 建立 Windows restricted-token runner 基线，再补 Windows 平台 enforce evidence；或按 P6 接外部 harness 的 RuntimeEvent adapter，不再重复补已完成的 P5/P7/P8 baseline。
...
P1/P2/P3/P4/P5/P7/P8 骨架已经进入 App Server / RuntimeCore current crates 与 Workbench current projection：真实 runtime backend tool stream 会派生 `file.read/file.changed/patch/command/test/permission/sandbox/action` 基础 coding events，并经 schema / sequence / tool lifecycle guard 入库；`patch-apply`、output/checkpoint refs、历史 hydrate、active read model、policy/sandbox/network diagnostics、provider/model routing、sandbox/process lifecycle owner、App Server `executionProcess/*`、unified exec canonical command/test、Workbench controls、GUI coding input smoke、production mock fallback fail-closed 与 legacy command 防回流契约都已具备 current facts。P2-A 的跨 backend bounded output 也已完成。后续实施不要回到纯文档、旧 UI、旧命令 inventory 或重复 output owner；下一刀直接进入正式 verifier/distribution evidence，外部 Agent host 消费合同等真实 production adapter 出现后再做。

下次恢复入口固定为当前工作树的 Pier/DeepSWE verifier 与正式 distribution evidence：先读 `internal/roadmap/coding/README.md` 的“下次从这里开始”，再核对 `scripts/harness/`、`packages/cli/`、Forge resource verifier、`.github/workflows/` 与 `internal/exec-plans/evidence/`。P2-A/P2-B/P2-C current owner 已完成；第一目标不是新增 UI、文档、output owner 或伪造外部 Agent host，而是生成绑定当前 tree identity 的机器可读证据，并在 Windows runner 复验 restricted-token 大输出。
